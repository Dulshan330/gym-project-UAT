...
const AllWorkoutSchedulePage = () => {
  ...
  useEffect(() => {
    async function loadUsers() {
      setLoading(true);
      const users = await fetchMemberDetails("Active");
      if (users) {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Button
            ...
            onClick={() => {
              router.push(
                `/dashboard/workouts/all-schedules/add-schedule?member=${user.id}`
              );
            }}
          >
            <PlusIcon /> Make a Schedule
          </Button>
        );
      },
    },
    ...
    <div className="p-4 w-full space-y-6 max-h-screen h-full flex flex-col">
      <div className="flex justify-between items-center">
        <div className="space-y-2">
          <p className="text-2xl font-semibold">Schedule Assignment</p>
...
export default AllWorkoutSchedulePage;
