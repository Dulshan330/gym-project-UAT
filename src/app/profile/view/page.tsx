...
const ProfileViewPage = () => {
  ...
  const fetchUserData = async () => {
    setLoading(true);
    try {
      const userData = await fetchUser(Number(member.id));
      if (userData) {
        setUser({
          id: userData.id,
      ...
      const fitnessData = await fetchSingleMemberDetails(Number(member.id));
      if (fitnessData) {
        setMemberFitnessData(fitnessData);
      }
      const trainerData = await fetchSingleTrainerDetails(Number(member.id));
      if (trainerData) {
        setTrainerProfile(trainerData);
      }
      const staffData = await fetchSingleStaffDetails(Number(member.id));
      if (staffData) {
        setStaffProfile(staffData);
      }
      const memberPackageData = await getAssignedPackage(Number(member.id));
  ...
  return (
    <>
      <div className="flex justify-between m-12 items-center max-md:m-4">
        <h2 className="text-2xl font-semibold m-4 max-md:text-xl">
          Profile Information
        </h2>
        <Button
          variant={"ghost"}
